function [opt_train,opt_test]=fet_mean_right()
...
% right sensors mean
for k=1:10
s_mean_right{1,k}=mean_acc(Sorted_right,min(find(Sorted_right(:,1)==47+k)),((max(find(Sorted_right(:,1)==47+k))/64)*64)-96,2);
end

for k=1:10
s_mean_right{2,k}=mean_acc(Sorted_right,min(find(Sorted_right(:,1)==47+k)),((max(find(Sorted_right(:,1)==47+k))/64)*64)-96,5);
end

for k=1:10
s_mean_right{3,k}=mean_acc(Sorted_right,min(find(Sorted_right(:,1)==47+k)),((max(find(Sorted_right(:,1)==47+k))/64)*64)-96,8);
end
...
for k=1:10
feature_bucket_right(min(find(feature_bucket_right(:,1)==k)):max(find(feature_bucket_right(:,1)==k)),2:10)=function_bucket_temp(k,len,s_mean_right);
end
