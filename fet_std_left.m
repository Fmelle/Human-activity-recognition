function [opt_train, opt_test]=fet_std_left()
...
clean_left(:,8:10)=left_classall_clean(:,45:47);
...
% left sensors std
for k=1:10
s_std_left{1,k}=std_acc(Sorted_left,min(find(Sorted_left(:,1)==47+k)),((max(find(Sorted_left(:,1)==47+k))/64)*64)-96,2);
end

for k=1:10
s_std_left{2,k}=std_acc(Sorted_left,min(find(Sorted_left(:,1)==47+k)),((max(find(Sorted_left(:,1)==47+k))/64)*64)-96,5);
end

for k=1:10
s_std_left{3,k}=std_acc(Sorted_left,min(find(Sorted_left(:,1)==47+k)),((max(find(Sorted_left(:,1)==47+k))/64)*64)-96,8);
end
...
for k=1:10
feature_bucket_left(min(find(feature_bucket_left(:,1)==k)):max(find(feature_bucket_left(:,1)==k)),2:10)=function_bucket_temp(k,len,s_std_left);
end
